...
new Story({
  ...
  enterHandler: (story, panel) => {
    if(panel === 3) {
      d3.csv("./assets/data/forest.csv", row => ({ year: +row.year, forest_area: +row.forest_area }))
        .then(data => {
    ...
    if(panel === 4) {
      d3.csv("./assets/data/co2.csv", row => ({ year: +row.year, co2: +row.co2 }))
        .then(data => {
    ...
    if(panel === 5) {
      d3.csv("./assets/data/co2_fuel.csv", rows => ({year: +rows.Year, gas: +rows.gas, oil: +rows.oil, coal: +rows.coal}))
        .then(data => {
